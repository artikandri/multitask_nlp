...
from multitask_nlp.settings import HOASA_ABSA_AIRY_DATA

ANNOTATION_COLUMNS = ['ac','air_panas','bau','general', 'sunrise_meal', 'tv', 'wifi','kebersihan', 'linen', 'service']
...
class HoasaAbsaAiryDataModule(BaseDataModule):
    def __init__(
        self,
        **kwargs,
    ):
        ...
        self.data_dir = HOASA_ABSA_AIRY_DATA
    ...
    @property
    def class_dims(self):
        return [3] * len(ANNOTATION_COLUMNS)

    @property
    def task_name(self):
        return "hoasa_absa_airy"
    ...
    def _get_annotations(self, df):
        for column in ANNOTATION_COLUMNS:
            df[column] = df[column].map(_CLASS_MAPPING)
        df = df.drop(['split', 'text'], axis=1)
        df.loc[:, 'annotator_id'] = 0
        return df

    @staticmethod
    def _load_df(directory: Path) -> pd.DataFrame:
        df = pd.read_csv(directory, delimiter=",", engine="python")
        df['text'] = df['review'] 
        return df
